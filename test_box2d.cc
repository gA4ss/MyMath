...
bool check_box_overlap_slow(const Box2d &box1, const Box2d &box2,
  ...
  *ambiguous = false;
  for (int k = 0; k < 4; ++k) {
    ...
    box1.get_all_corners(&c1);
    box2.get_all_corners(&c2);
...
int main(int argc, char* argv[]) {
  ...
  TEST_START("has_overlap");
  {
    ...
    EXPECT_FALSE(box1.has_overlap(box3));
    EXPECT_FALSE(box1.has_overlap(box4));
    EXPECT_FALSE(box2.has_overlap(box4));
    EXPECT_FALSE(box3.has_overlap(box4));

    EXPECT_TRUE(box1.has_overlap(LineSegment2d({0, 0}, {1, 1})));
    EXPECT_TRUE(box1.has_overlap(LineSegment2d({0, 0}, {3, 3})));
    EXPECT_TRUE(box1.has_overlap(LineSegment2d({0, -3}, {0, 3})));
    EXPECT_TRUE(box1.has_overlap(LineSegment2d({4, 0}, {-4, 0})));
    EXPECT_TRUE(box1.has_overlap(LineSegment2d({-4, -4}, {4, 4})));
    EXPECT_TRUE(box1.has_overlap(LineSegment2d({4, -4}, {-4, 4})));
    EXPECT_FALSE(box1.has_overlap(LineSegment2d({-4, -4}, {4, -4})));
    EXPECT_FALSE(box1.has_overlap(LineSegment2d({4, -4}, {4, 4})));
  }
  TEST_END("has_overlap");

  TEST_START("get_aabox");
  {
    AABox2d aabox1 = box1.get_aabox();
    AABox2d aabox2 = box2.get_aabox();
    AABox2d aabox3 = box3.get_aabox();
    AABox2d aabox4 = box4.get_aabox();
  ...
  {
    EXPECT_NEAR(box1.distance_to({3, 0}), 1.0, 1e-5);
    EXPECT_NEAR(box1.distance_to({-3, 0}), 1.0, 1e-5);
    EXPECT_NEAR(box1.distance_to({0, 2}), 1.0, 1e-5);
    EXPECT_NEAR(box1.distance_to({0, -2}), 1.0, 1e-5);
    EXPECT_NEAR(box1.distance_to({0, 0}), 0.0, 1e-5);
    EXPECT_NEAR(box1.distance_to({0, 1}), 0.0, 1e-5);
    EXPECT_NEAR(box1.distance_to({1, 0}), 0.0, 1e-5);
    EXPECT_NEAR(box1.distance_to({0, -1}), 0.0, 1e-5);
    EXPECT_NEAR(box1.distance_to({-1, 0}), 0.0, 1e-5);

    EXPECT_NEAR(box1.distance_to(LineSegment2d({-4, -4}, {4, 4})), 0.0, 1e-5);
    EXPECT_NEAR(box1.distance_to(LineSegment2d({4, -4}, {-4, 4})), 0.0, 1e-5);
    EXPECT_NEAR(box1.distance_to(LineSegment2d({0, 2}, {4, 4})), 1.0, 1e-5);
    EXPECT_NEAR(box1.distance_to(LineSegment2d({2, 2}, {3, 1})),
  ...
  {
    EXPECT_TRUE(box1.is_point_in({0, 0}));
    EXPECT_TRUE(box1.is_point_in({1, 0.5}));
    EXPECT_TRUE(box1.is_point_in({-0.5, -1}));
    EXPECT_TRUE(box1.is_point_in({2, 1}));
    EXPECT_FALSE(box1.is_point_in({-3, 0}));
    EXPECT_FALSE(box1.is_point_in({0, 2}));
    EXPECT_FALSE(box1.is_point_in({-4, -2}));
  }
  TEST_END("is_point_in");

  TEST_START("is_point_on_boundary");
  {
    EXPECT_FALSE(box1.is_point_on_boundary({0, 0}));
    EXPECT_FALSE(box1.is_point_on_boundary({1, 0.5}));
    EXPECT_TRUE(box1.is_point_on_boundary({-0.5, -1}));
    EXPECT_TRUE(box1.is_point_on_boundary({2, 0.5}));
    EXPECT_TRUE(box1.is_point_on_boundary({-2, 1}));
    EXPECT_FALSE(box1.is_point_on_boundary({-3, 0}));
    EXPECT_FALSE(box1.is_point_on_boundary({0, 2}));
    EXPECT_FALSE(box1.is_point_on_boundary({-4, -2}));
  }
  TEST_END("is_point_on_boundary");

  TEST_START("rotate_from_center_and_shift");
  {
    Box2d box({0, 0}, 0, 4, 2);
    std::vector<Vec2d> corners;
    EXPECT_NEAR(box.heading(), 0.0, 1e-5);
    box.rotate_from_center(M_PI_2);
  ...
  TEST_END("rotate_from_center_and_shift");

  TEST_START("test by random");
  {
    ...
    for (int iter = 0; iter < 10000; ++iter) {
      ...
      bool overlap = check_box_overlap_slow(box1, box2, &ambiguous);
      if (!ambiguous) {
        EXPECT_EQ(overlap, box1.has_overlap(box2));
      }
    }
    for (int iter = 0; iter < 100; ++iter) {
      ...
      for (int iter2 = 0; iter2 < 1000; ++iter2) {
        ...
        EXPECT_EQ(box.is_point_in({x, y}), poly.is_point_in({x, y}));
        EXPECT_EQ(box.is_point_on_boundary({x, y}), poly.is_point_on_boundary({x, y}));
        EXPECT_NEAR(box.distance_to({x, y}), poly.distance_to({x, y}), 1e-3);

        const double other_x = random_double(-20, 20);
        const double other_y = random_double(-20, 20);
        const LineSegment2d segment({x, y}, {other_x, other_y});
        EXPECT_EQ(box.has_overlap(segment), poly.has_overlap(segment));
        EXPECT_NEAR(box.distance_to(segment), poly.distance_to(segment), 1e-3);
      }
    }
  }
  TEST_END("test by random");
}
