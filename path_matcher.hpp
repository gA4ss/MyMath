#ifndef COMMON_MYMATH_PATH_MATCHER_HPP
#define COMMON_MYMATH_PATH_MATCHER_HPP
...
public:
  ...
  static PathPoint match_to_path(const std::vector<PathPoint>& reference_line,
    ...
    return find_projection_point(reference_line[index_start],
                                 reference_line[index_end], x, y);
  }

  static std::pair<double, double> get_path_frenet_coordinate(
    const std::vector<PathPoint>& reference_line, const double x,
    const double y) {
    auto matched_path_point = match_to_path(reference_line, x, y);
  ...
  static PathPoint match_to_path(const std::vector<PathPoint>& reference_line,
    ...
    return interpolate_using_linear_approximation(*(it_lower - 1), *it_lower, s);
  }

private:
  static PathPoint find_projection_point(const PathPoint& p0, const PathPoint& p1,
                                         const double x, const double y) {
    double v0x = x - p0.x();
    double v0y = y - p0.y();
    ...
    return interpolate_using_linear_approximation(p0, p1, p0.s() + delta_s);
  }
};
